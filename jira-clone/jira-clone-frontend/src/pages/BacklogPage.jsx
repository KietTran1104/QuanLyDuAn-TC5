...
export default function BacklogPage({ onLogout }) {
  ...
  const [subtaskCreateMode, setSubtaskCreateMode] = useState(false)
  ...
  const getSprintIssues = (sprintId) => issues.filter(i => i.sprintId === sprintId && i.parentIssueId == null)
  const getBacklogIssues = () => issues.filter(i => !i.sprintId && i.parentIssueId == null)
  ...
  const renderIssueRow = (iss) => (
    <BacklogDraggableIssue 
        key={iss.id} 
        issue={iss} 
        onClick={() => { setSelectedIssueId(iss.id); setSubtaskCreateMode(false); }} 
        onAddSubtask={() => { setSelectedIssueId(iss.id); setSubtaskCreateMode(true); }}
    />
  )
    ...
    <Layout projectId={id || 'WEB'} onLogout={onLogout}>
      ...
      {selectedIssueId && (
        <IssueDetailDrawer 
          ...
          statuses={statuses}
          onClose={() => { setSelectedIssueId(null); setSubtaskCreateMode(false); }}
          autoFocusSubtask={subtaskCreateMode}
