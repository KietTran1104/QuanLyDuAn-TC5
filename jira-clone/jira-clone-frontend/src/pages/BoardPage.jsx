...
export default function BoardPage({ onLogout }) {
  ...
  const handleIssueUpdated = (updatedIssue) => {
    if (updatedIssue && updatedIssue.id) {
      ...
      // If no issue passed (e.g., deleted or subtask added), refetch the board
      fetchBoardData()
    }
  }
  ...
  return (
    <Layout projectId={id || 'WEB'} onLogout={onLogout}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#5E6C84', marginRight: '8px' }}>Lọc theo:</span>
          {['epic', 'story', 'task', 'bug'].map(type => (
            <button
              ...
              style={{
                padding: '4px 12px',
                border: '1px solid',
                borderColor: selectedTypes.includes(type) ? '#0C66E4' : '#DFE1E6',
                borderRadius: '16px',
                backgroundColor: selectedTypes.includes(type) ? '#E9F2FF' : 'white',
                color: selectedTypes.includes(type) ? '#0C66E4' : '#42526E',
                cursor: 'pointer',
                fontSize: '13px',
                fontWeight: '500',
          ...
          {selectedTypes.length > 0 && (
            <span 
              ...
              style={{ fontSize: '13px', color: '#0C66E4', cursor: 'pointer', marginLeft: '8px', fontWeight: '500' }}
            >
              Xóa bộ lọc
            </span>
          )}
            ...
            <SortableContext items={columns.map(c => `col-${c.id}`)} strategy={horizontalListSortingStrategy}>
              {columns.map((col) => {
                ...
                const columnIssues = issues.filter(i => {
                  if (Number(i.statusId) !== colId) return false;
                  if (selectedTypes.length > 0 && !selectedTypes.includes(i.type?.toLowerCase())) return false;
                  return true;
                })
