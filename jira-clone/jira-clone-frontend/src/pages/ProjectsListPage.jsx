...
export default function ProjectsListPage({ onLogout }) {
  ...
  useEffect(() => {
    // Khóa lại tất cả các dự án mỗi khi quay lại danh sách dự án
    Object.keys(sessionStorage).forEach(key => {
      if (key.startsWith('project_unlock_')) {
        ...
        {projects.map(project => {
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#172B4D', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {project.name}
                  {project.pin && (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#626F86" strokeWidth="2" title="Dự án có mã PIN">
