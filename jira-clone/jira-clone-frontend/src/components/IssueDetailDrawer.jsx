...
export default function IssueDetailDrawer({ projectId, issueId, onClose, onIssueUpdated, statuses, onNavigate, autoFocusSubtask }) {
  ...
  const handleUpdateField = async (fieldData) => {
    try {
      ...
      if (res.ok) {
        setIssue(res.data)
        if (onIssueUpdated) onIssueUpdated({ ...issue, ...fieldData })
      } else {
        addToast(res.data?.message || 'Cập nhật thất bại', 'error')
      }
    } catch (e) {
      addToast('Lỗi hệ thống khi cập nhật', 'error')
    }
  }
    ...
    if (e.key === 'Enter') {
      ...
      try {
        const payload = {
          ...
          type: 'subtask',
          statusId: statuses && statuses.length > 0 ? statuses[0].id : issue.statusId,
        ...
        if (res.ok) {
          ...
          if (onIssueUpdated) onIssueUpdated() // Refresh parent to show subtask count/icons
        } else {
          addToast(res.data?.message || 'Lỗi tạo Sub-task', 'error')
        }
      } catch (e) {
