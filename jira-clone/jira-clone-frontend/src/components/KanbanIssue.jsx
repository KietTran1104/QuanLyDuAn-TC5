...
export default function KanbanIssue({ issue, onIssueClick, onDeleteIssue, isSubtask, parentKey }) {
  ...
  const style = {
    ...
    position: 'relative',
    marginLeft: isSubtask ? '24px' : '0',
    width: isSubtask ? 'calc(100% - 24px)' : '100%',
    padding: isSubtask ? '8px 12px' : '12px',
  ...
  const getTypeColor = (type) => {
    switch(type?.toLowerCase()) {
      ...
      case 'subtask': return '#4C9AFF'; // Light blue for subtasks
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: isSubtask ? '13px' : '14px', color: '#172B4D', marginBottom: isSubtask ? '8px' : '12px', lineHeight: '1.4', paddingRight: isHovered ? '24px' : '0' }}>
        {isSubtask && (
          <span style={{ color: '#0052CC', marginRight: '6px', fontWeight: '500' }}>
            [{parentKey}]
          </span>
        )}
        {issue.summary}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <div style={{ width: '16px', height: '16px', backgroundColor: getTypeColor(issue.type), borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
             {issue.type?.toLowerCase() === 'subtask' ? (
               <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
             ) : (
               <div style={{ width: '8px', height: '8px', backgroundColor: 'white', borderRadius: '2px' }}></div>
             )}
          </div>
          <span style={{ fontSize: '11px', color: '#626F86', fontWeight: '600' }}>{issue.issueKey}</span>
        </div>
