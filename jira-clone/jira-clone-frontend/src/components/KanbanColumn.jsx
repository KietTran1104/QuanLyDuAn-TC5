      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '100px', flex: 1 }}>
        <SortableContext items={issues.map(i => `task-${i.id}`)} strategy={verticalListSortingStrategy}>
          {(() => {
            ...
            // 1. Render parents and their subtasks in this column
            issues.filter(i => i.type?.toLowerCase() !== 'subtask').forEach(parent => {
              elements.push(
                <KanbanIssue 
                  key={parent.id} 
                  issue={parent} 
              ...
              // Find subtasks of this parent in THIS column
              issues.filter(s => s.type?.toLowerCase() === 'subtask' && s.parentIssueId === parent.id).forEach(sub => {
                elements.push(
                  <KanbanIssue 
                    key={sub.id} 
                    issue={sub} 
            ...
            // 2. Render subtasks whose parents are NOT in this column
            issues.filter(i => i.type?.toLowerCase() === 'subtask' && !renderedIds.has(i.id)).forEach(orphanSub => {
              elements.push(
                <KanbanIssue 
                  key={orphanSub.id} 
                  issue={orphanSub} 
                  ...
                  onDeleteIssue={onDeleteIssue} 
                  isSubtask={true}
                  parentKey={`Parent ID: ${orphanSub.parentIssueId}`} // Backend should ideally provide parentKey
                />
              );
            });
